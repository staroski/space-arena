...
public class CGUtils {
	...
	public static Shape[] createRotatedShapes(Shape original, double deltaDegrees) {
		...
		for (int i = 0; i < count; i++) {
			polygons[i] = rotateShape(original, i * deltaDegrees);
		}
		return polygons;
	}
	...
	public static BufferedImage loadBufferedImage(String resource) {
		return bufferize(loadImage(resource));
	}
	...
	public static Image loadImage(String resource) {
		return Toolkit.getDefaultToolkit().createImage(getBytes(resource));
	}
	...
	public static BufferedImage rotateImage(BufferedImage img, double degrees) {
		...
		// criar objeto de transformação da rotação
		AffineTransform transform = new AffineTransform();
		// rotacionar
